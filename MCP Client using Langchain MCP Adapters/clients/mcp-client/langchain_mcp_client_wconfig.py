...
This file implements a LangChain MCP client that:
  ...
  - Uses the OpenAI GPT API (via LangChain) to create a React agent with access to all tools.
...
from langchain_openai import ChatOpenAI                  # Wrapper for the OpenAI GPT API via LangChain
...
# ---------------------------
# OpenAI GPT LLM Instantiation
# ---------------------------
llm = ChatOpenAI(
    model="gpt-5",                         # Specify the OpenAI GPT model variant to use
...
async def run_agent():
    ...
    async with AsyncExitStack() as stack:
        ...
        # Create a React agent using the OpenAI GPT LLM and the list of aggregated tools
